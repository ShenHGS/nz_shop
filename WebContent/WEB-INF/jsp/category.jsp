  ...
  <nav class="navbar navbar-default navbar-static-top" role="navigation"
	...
	<div id="page-wrapper">
		...
		<%-- <div class="panel panel-default">
			<div class="panel-body">
				<form class="form-inline" method="get" 
				      action="${pageContext.request.contextPath }/category/getCategoryById.action">
					<div class="form-group">
						<label for="cid">商品类型名称</label> 
		...
		<div class="panel panel-default">
			<div class="panel-body">
				<form class="form-inline" method="get" 
				      action="${pageContext.request.contextPath }/category/list.action">
					<div class="form-group">
						<label for="cname">商品类型名称</label> 
		...
		<a href="#" class="btn btn-primary" data-toggle="modal" 
		           data-target="#newUserDialog" onclick="clearCategory()">新建</a>
		<div class="row">
			<div class="col-lg-12">
				<div class="panel panel-default">
						...
						<tbody>
							<c:forEach items="${page.rows}" var="row">
								<tr>
									...
									<td>${row.cname}</td>
									<td>
										<a href="#" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#UserEditDialog" onclick= "editCategory(${row.cid})">修改</a>
										<a href="#" class="btn btn-danger btn-xs" onclick="deleteCategory(${row.cid})">删除</a>
									</td>
								</tr>
							</c:forEach>
						</tbody>
					...
					<div class="col-md-12 text-right">
						<abkj:page url="${pageContext.request.contextPath }/category/list.action" />
					</div>
					<!-- /.panel-body -->
				</div>
				<!-- /.panel -->
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form class="form-horizontal" id="new_category_form">
					<div class="form-group">
						...
						<div class="col-sm-10">
							<input type="text" class="form-control" id="new_cname" placeholder="商品类型名称" name="cname" />
						</div>
					</div> 
				</form>
			</div>
			<div class="modal-footer">
				...
				<button type="button" class="btn btn-primary" onclick="insertCategory()">添加</button>
			</div>
		</div>
	</div>
</div>
...
<div class="modal fade" id="UserEditDialog" tabindex="-1" role="dialog"
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form class="form-horizontal" id="edit_category_form">
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<input type="text" class="form-control" id="edit_cname" placeholder="商品类型名称" name="cname" />
						</div>
					</div>
				</form>
			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
				<button type="button" class="btn btn-primary" onclick="updateCategory()">保存修改</button>
			</div>
		</div>
	</div>
</div>
...
//清空新建客户窗口中的数据
	...
	$.post("<%=basePath%>category/insert.action",
	$("#new_category_form").serialize(),function(data){
	        if(data =="OK"){
	            alert("商品类型添加成功！");
	...
	function editCategory(cid) {
	    $.ajax({
	        type:"get",
	        url:"<%=basePath%>category/getCategoryById.action",
	        data:{"cid":cid},
	...
	function updateCategory() {
		$.post("<%=basePath%>category/update.action",$("#edit_category_form").serialize(),function(data){
			if(data =="OK"){
				alert("商品类型更新成功！");
				window.location.reload();
			}else{
	...
	$.post("<%=basePath%>category/delete.action",{"cid":cid},
	function(data){
	            if(data =="OK"){
	                alert("商品类型删除成功！");
	                window.location.reload();
